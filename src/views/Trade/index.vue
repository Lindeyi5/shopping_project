  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="addre in addressInfo"
        :key="addre.id"
        @click="changeAddress(addressInfo, addre)"
      >
        <span class="username" :class="{ selected: addre.isDefault == 1 }">{{
          addre.consignee
        }}</span>
        <p>
          <span class="s1">{{ addre.fullAddress }}</span>
    ...
    <div class="sub clearFix" style="height: 55px">
...
export default {
  ...
  mounted() {
    this.$store.dispatch('getReceInfo')
  ...
  methods: {
    changeAddress(addressInfo, addre) {
    ...
    async submitorder() {
      ...
      try {
        let result = await this.$API.submitOrderInfo(tradeNo, data)
        if (result.data == 200) {
          this.orderId = result.data
        } else {
          // this.tardeNum=result.data
          this.orderId = 10552
